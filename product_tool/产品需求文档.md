...
| **架构** | Next.js 前端（Vercel）+ FastAPI 后端（Railway） |
| **当前状态** | ✅ 通用解析器上线（三层策略：KV→表格→内容驱动，评分择优）。✅ PDF三层策略解析（布局检测→内容驱动→评分择优）。✅ DOCX三层策略解析（表头匹配→内容驱动→段落提取→评分择优）。✅ 图片按列过滤（检测"产品图片"列，排除包装图/适合图）。✅ 货币自动识别（FOB/USD表头→$显示，RMB→¥显示）。✅ 备注附着&合并单元格处理。✅ 产品库/报价历史自动刷新。✅ 报价历史下载/删除。✅ 产品库批量删除 |。✅ PostgreSQL 迁移(Supabase)。✅ Creem 订阅支付(接入/Webhook/自动升降级)。✅ 双Token认证(access+refresh httpOnly cookie)。✅ 前端配额展示(上传进度+产品库计数)（含change-password修复）
| **有网页版吗？** | ✅ 有。Next.js Web 界面 + FastAPI 后端，已部署 |
| **要钱吗？** | 免费体验（3个文件）+ 注册免费 + 专业版 ¥39/月（预售价） |
...
所有解析器统一为三层策略架构，评分函数自动选择最佳结果：

| 解析器 | 策略 A | 策略 B | 策略 C | 评分函数 |
|--------|--------|--------|--------|---------|
...
评分规则：`产品数 × 0.3 + 有型号数 × 0.3 + 有价格数 × 0.2 + 型号多样性 × 0.2 − 标签惩罚`（Image/Description 等被当产品名则扣分）。
