...
后端 parse_file():
  ...
  Step 3 - 择优选择:
    判断逻辑（main.py:1146-1189）:
    ├─ 专用有型号 + 通用无型号 → 专用胜出
    ├─ 专用有价格 + 通用无价格 → 专用胜出
    ├─ 都有型号+价格 → 比较 (模型数 + 价格数) 之和
    ├─ 都有价格但无型号 → 选价格多的
    └─ 默认 → 保留通用结果
...
### 4.3 API 层 Step 3 择优比较

```python
# main.py parse_file() 核心比较逻辑

# 有模型 + 无型号 → 专用胜出
# 有价格 + 无价格 → 专用胜出
# 都有型号+价格 → (spec_model_cnt + spec_price_cnt) vs (uni_model_cnt + uni_price_cnt)
# 都有价格无型号 → 价格多的胜出
```

评价指标:
- `_has_model(d)`: model 列非空行数 >0
- `_model_score(d)`: model 列非空行数
- 价格指标: price_rmb notna() 求和
